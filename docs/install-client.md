...
| Systemd unit | `autoproxy-client.service` | Runs `autoproxy-client run`, which brings everything up and then logs the handshake age every 30 seconds. After 3 minutes without a handshake it moves the tunnel to a new local UDP port (a path stuck on one port recovers without a restart), and it does the same when two version reports to the VPS in a row time out while the handshake still looks fresh (a path that passes handshakes but drops traffic). After 10 minutes without a handshake it exits so systemd restarts it. A restart or `SIGTERM` leaves the tunnel itself up so in-progress player sessions survive a service restart. |
